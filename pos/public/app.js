...
function renderCollectionCounts(collectionCounts = {}) {
  els.collectionCounts.innerHTML = Object.entries(collectionCounts)
    .map(([key, value]) => `<div class="count-tile"><span>${key}</span><strong>${value}</strong></div>`)
    .join('');
}
...
function renderInventory(changedIds = new Map()) {
  ...
  const rows = state.inventory
    .filter((item) => {
      ...
      return [item.medicationName, item.sku, item.barcodeData, item.batchLotNumber]
        .some((value) => String(value || '').toLowerCase().includes(filter));
    })
    .sort((a, b) => a.medicationName.localeCompare(b.medicationName))
    .map((item) => {
      const change = changedIds.get(item.id);
      const rowClass = change ? (change.delta >= 0 ? 'flash-up' : 'flash-down') : '';
      const stockCell = change
        ? `<span class="delta ${change.delta >= 0 ? 'up' : 'down'}">${item.currentStock} (${change.delta >= 0 ? '+' : ''}${change.delta})</span>`
        : item.currentStock;
  ...
  els.inventoryBody.innerHTML = rows || '<tr><td colspan="8">No matching items.</td></tr>';
}
  ...
  els.activityFeed.innerHTML = items
    .map((item) => `
      <div class="activity-item">
        <div class="row"><strong>${item.medicationName}</strong><span class="delta ${item.delta >= 0 ? 'up' : 'down'}">${item.delta >= 0 ? '+' : ''}${item.delta}</span></div>
        <div class="row"><span>${item.changeType}</span><small>${fmtDate(item.changedAt)}</small></div>
...
async function loadInitialData() {
  const [configResponse, statusResponse, activityResponse, bootstrapResponse] = await Promise.all([
  ...
  renderCollectionCounts(bootstrapResponse.collectionCounts || statusResponse.collectionCounts || {});
}

async function pollChanges() {
  try {
    const [changesResponse, statusResponse, activityResponse] = await Promise.all([
